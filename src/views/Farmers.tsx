...
export default function Farmers({ userRole }: { userRole?: string }) {
  ...
  const confirmDelete = async () => {
    ...
    const { error } = await supabase.from('farmers').delete().eq('id', farmerToDelete.id);
    if (!error) {
            ...
            <div className="p-6">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className="text-xl font-bold text-gray-900 mb-2">Delete Farmer</h3>
                  <p className="text-sm text-gray-600 leading-relaxed">
                    Are you sure you want to delete the farmer <span className="font-bold text-gray-900 uppercase">{farmerToDelete?.name}</span>? This action cannot be undone and will permanently remove their record from the system.
                  </p>
                </div>
              </div>
            </div>
