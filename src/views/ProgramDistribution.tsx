...
export default function ProgramDistribution({ userRole }: { userRole?: string }) {
  ...
  const handleDelete = (dist: Distribution) => {
  ...
  const confirmDelete = async () => {
    ...
    const { error } = await supabase.from('distributions').delete().eq('id', distributionToDelete.id);
    if (!error) {
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600 leading-relaxed">
                    Are you sure you want to delete the program <span className="font-bold text-gray-900 uppercase">{distributionToDelete?.program_name}</span>? This action cannot be undone and will permanently remove this record from the system.
                  </p>
                </div>
              </div>
            </div>
