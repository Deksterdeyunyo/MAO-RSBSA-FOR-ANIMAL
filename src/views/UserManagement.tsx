...
export default function UserManagement() {
  ...
  const confirmDelete = async () => {
    ...
    const { error } = await supabase.from('users_management').delete().eq('id', userToDelete.id);
    if (!error) {
                ...
                <tr key={user.id} className={`hover:bg-gray-50 transition-colors ${selectedIds.includes(user.id) ? 'bg-green-50/50' : ''}`}>
                  ...
                  <td className="px-6 py-4 text-right text-sm font-medium">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button onClick={() => handleDelete(user)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-md transition-colors" title="Delete">
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600 leading-relaxed">
                    Are you sure you want to delete the user <span className="font-bold text-gray-900 uppercase">{userToDelete?.name}</span>? This action cannot be undone and will permanently remove their access to the system.
                  </p>
                </div>
              </div>
            </div>
